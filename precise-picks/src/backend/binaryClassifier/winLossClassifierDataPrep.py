...
def getSeasonScheduleFrame(seasons,seasonType): 
    ...
    for season in seasons:
       for id in teamLookup['id']:
          time.sleep(1)
          schedule_data.append(getRegularSeasonSchedule(season, id, seasonType))
       print(f"season {season} added to schedule_data") 
    scheduleFrame = pd.concat(schedule_data, ignore_index=True)
